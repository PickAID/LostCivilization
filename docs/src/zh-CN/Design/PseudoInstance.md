...
本页定义第一版遗址现场的运行模型。我们把 `伪副本` 限定为：以正式遗址记录为起点，在原世界宿主结构上建立一段受控、可回收、可清场的现场运行态。它不是独立维度，也不是某个右键动作的放大版。
...
## 长期真相与短期运行态 {#ledger-and-runtime-split}

伪副本必须与世界账本拆开。`SavedData` 负责长期真相，伪副本只负责短生命周期状态。

| 层 | 主键 | 保存什么 |
| --- | --- | --- |
| 跨阶段引用 | `SiteRef` | 正式勘探、激活和回收之间的统一引用 |
| 遗址账本 | `SiteCoordinate` | 遗址身份、锚点、生命周期、正式记录 |
| 运行态注册表 | `SiteCoordinate` | 当前是否激活、由谁占用、当前阶段、局部缓存 |
| 区块副索引 | `long chunkKey` | 这个 chunk 当前覆盖了哪些现场 |

`ActivationService` 先用 `SiteRef` 解析到账本记录，再归一到同一条 `SiteCoordinate`。长期记录与运行态一旦混写，区块卸载、撤退结算和重复激活就会互相污染。
...
`ChunkEvent.Unload` 的职责也要限定清楚。它负责清掉某个 chunk 上的临时缓存、局部生成物索引和渲染挂钩，不负责删掉整条遗址，也不负责改写账本状态。

## 激活入口与现场服务 {#activation-entry-and-runtime-services}

激活入口可以来自道具、机器或现场装置，但这些入口都不应该各自实现一套现场启动逻辑。第一版统一收口到 `ActivationService`。
...
| `SiteFeedbackService` | 雾、音效、提示和局部表现 | 不持有权威状态 |
...
5. `RecoveryService` 结算结果、写回账本、注销运行态并清空局部索引。

运行态结束时删除的是运行态注册，不是遗址真相。遗址是否完成、失效或允许再次进入，必须回到账本层判定。
