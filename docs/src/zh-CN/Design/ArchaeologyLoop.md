...
本页定义遗址主循环的正式边界。我们只回答三件事：遗址何时进入正式实例，现场各阶段按什么顺序接力，以及哪些数据必须进入世界账本。前期发现与正式勘探的细节分界见 `ModdingDeveloping/Design/Survey`；本页只保留全链路规则。
...
| 阶段 | 输入 | 输出 | 边界 |
| --- | --- | --- | --- |
| 前期发现 | 环境载体、刷扫交互、线索节点 | 线索、材料、提示信息 | 不创建 `SiteRef`，不写正式账本 |
...
| 共鸣 | 当前现场、遗物状态、doctrine | 状态修正、结算修正 | 不替代回收入账 |
| 回收 | 现场结算、共鸣结果 | 遗物结果、记录、长期知识 | 不重写定位真相 |
...
4. 现场运行态只保存短生命周期状态。压力、守卫、阶段推进和局部同步属于运行态；遗址身份、锚点和生命周期真相属于账本。这样区块卸载不会误删遗址。
5. 共鸣位于现场运行之后、回收之前。它读取当前现场与遗物状态，产出状态修正和结算修正；如果提前到现场之前，共鸣会退化成静态词条。
6. 回收负责把一次行动写回长期层。遗物结果、碎片、记录和 tooltip/图鉴推进都在这里落账；否则主循环无法闭合。
...
| 账本入口 | `ServerLevel.getDataStorage()` + `DimensionDataStorage.computeIfAbsent(...)` | 创建或读取维度级 `SavedData` | 正式记录必须进入稳定持久化入口 |

结构提供宿主和空间边界；群系提供类型偏向；账本把这次定位结果固定为一条可追踪记录。这三层不能互相替代。

## 遗址账本 {#site-ledger}

遗址账本不是可选装饰。我们至少需要一份能够回答以下问题的权威记录：

- 这座遗址位于哪个维度和哪个锚点。
- 它被识别成哪一种遗址类型。
- 它当前处于发现、待激活、运行中、已完成还是已失效。
- 哪个玩家或哪个激活上下文正在占用它。
...
| 字段 | 作用 |
| --- | --- |
| `recordsByCoordinate` | 遗址真相表，回答某个锚点当前是什么状态 |
| `coordinatesByChunk` | 查询索引，服务区块同步、局部缓存和覆盖范围检查 |
| `activeSiteByOwner` | 占用索引，回答某个玩家或上下文当前绑定了哪一座遗址 |

这三张表分别对应“真相”“查询”“运行中绑定”。如果把三者混成一个 map，后面要么查不快，要么状态会互相覆盖。
...
| 账本真相不随区块卸载删除 | 区块生命周期不是遗址生命周期 |
| 运行态必须能反查到账本记录 | 结算阶段必须知道结束的是哪一座遗址 |
...
这样排序的原因很直接。结构提供稳定宿主和空间边界；群系只提供环境语义。如果让群系先行，实例主键会变成软条件，后续账本、同步和注册都不稳定。
...
只要这条切片能稳定走完“发现 -> 入账 -> 激活 -> 运行 -> 共鸣 -> 回收”，主循环就算成立。
