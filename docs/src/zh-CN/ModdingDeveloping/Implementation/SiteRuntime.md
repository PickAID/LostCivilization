---
title: 现场运行态
description: 现场运行态的 Forge 实现契约，包含世界账本、chunk 辅助数据、同步与保存检查点。
priority: 30
---
...
```mermaid
flowchart LR
    Ledger["世界账本 SavedData"] --> Registry["活跃运行态 Registry"]
...
## `SiteLedgerSavedData` 建议骨架 {#site-ledger-saved-data-skeleton}
...
## 当前推荐的对象边界 {#current-recommended-object-boundaries}
