...
| `logs/`、`saves/`、临时导出 | 联调证据 | 不作为长期项目真相 |

如果文件系统和页面内容冲突，先查明当前实例为什么变了，再改永久页面；不要反过来拿旧文档覆盖现状。
...
`docs/package.json` 已把 `packageManager` 固定为 `yarn@1.22.22`。因此日常文档命令以 `yarn` 为准。
...
这样读的原因很简单。先确认项目边界，再看运行时、设计、整合和贡献规则，避免一开始就掉进局部页面。
...
`ModdingDeveloping` 不讨论 KubeJS 或 pack 侧脚本；那部分应该写回 `Modpacking`。这一条边界已经锁定，不再混写。
